...
function WordSearchView(matrix, list, gameId, listId, instructionsId,listKo,wordLocTemp,matrixTemp) {
	 ...
	 */
	 this.setUpView = function() {
		...
		createListOfWords(list, listId,listKo);
...
// Biến lưu trữ màu đã chọn lần trước
var lastColor = null;

// Hàm chọn ngẫu nhiên một màu từ mảng, đảm bảo màu mới khác với màu lần trước
function getRandomColor() {
    ...
    // Cập nhật màu lần trước
    lastColor = newColor;
    return newColor;
}
	function createListOfWords(wordList, wordListId,wordListKo) {
		...
		for (var i = 0; i < wordList.length; i++) {
			...
			for (var j = 0; j < wordList[i].length; j++) {
				...
				word.attr({class: "listWord",id:wordList[i][j].replace(/\W/g, ""), text: wordList[i][j].replace(/\W/g, ""),onclick: "myFunction('" + wordList[i][j] + "','"+wordListKo[i][j]+"',1)"});

				//given text from it's respected list index
				word.text(wordList[i][j]);

				word.click(function() {
					var wordId = $(this).attr('id'); // Lấy id của phần tử đã click
					var wordText = $(this).text();   // Lấy text của phần tử đã click
					console.log("Clicked on word:", wordText, "with id:", wordId);
					WordSearchView.solveOne(wordLocTemp, matrixTemp, wordId);
				});
				word.appendTo(row);
			...
			for (var j = 0; j < wordList[i].length; j++) {
				...
				//they're given a list word class, and an attribute containing it's trimmed text (as in the puzzle)
				word.attr({class: "listWord",id:wordList[i][j].replace(/\W/g, ""), text: wordList[i][j].replace(/\W/g, ""),onclick: "myFunction('" + wordList[i][j] + "','"+wordListKo[i][j]+"',2)"});

				//given text from it's respected list index
				word.text(wordListKo[i][j]);
	 ...
	 */
	this.solveOne = function(wordLoc, matrix,id) {
		// updateTimer(1);
		...
		Object.keys(wordLoc).forEach(function(word) {  	
			...
			for (var k = 0, x = startX, y = startY; k < word.length; k++, x = incr[p](x, y).x, y = incr[p](x, y).y) {
				...
				$(select.cells + "[row = " + x + "][column = " + y + "]").css({'background-color': color,'opacity': '0.9'});	
			...
			if (elements.length > 1) {
				// Iterate over the elements NodeList and add the 'found' class to each element
			...
			// var decreaseAmount = Math.floor(countdownTime * 0.05);
            // countdownTime -= decreaseAmount;
	 ...
	 this.triggerMouseDrag = function() {	
		...
		function endMove() {
			...
			if (validWordMade(list, wordMade, instructionsId)) {
				///
				$(select.selected).css({'background-color': color});
				console.log(wordMade);
				console.log(list);
		...
		for (var i = 0; i < list.length; i++) {
			...
			for (var j = 0; j < list[i].length; j++) {
				...
				if (wordToCheck == trimmedWord ||
					...
					var elements = document.querySelectorAll('#'+trimmedWord);
							if (elements.length > 1) {
								var secondElement = elements[1]; // Lấy phần tử thứ hai trong danh sách
								var textContent = secondElement.textContent || secondElement.innerText;
								// In ra nội dung text của phần tử thứ hai
								
							}
							
							setTimeout(function() {
								// Mã bạn muốn thực thi sau khoảng trì hoãn
								speakText2(textContent);
